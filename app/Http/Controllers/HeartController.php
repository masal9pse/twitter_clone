...
class HeartController extends Controller
{
  public function heart(Comment $comment, Request $request)
  {
    ...
    $heart = Heart::create(['user_id' => $request->user_id, 'comment_id' => $comment->id]);
    // dd($request->user_id);
    $heartCount = count(Heart::where('comment_id', $comment->id)->get());
  ...
  {
    $heart = Heart::where('user_id', $request->user_id)->where('comment_id', $comment->id)->first();
    $heart->delete();
